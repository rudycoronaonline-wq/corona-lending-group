Place your blog images in this folder. The blog expects these filenames (paths are used in frontmatter):

Main blog page (hero banner at top of /blog):
  blog-hero.png

Fallback when a post has no image (optional):
  blog-placeholder.png

Simple names (use as-is):
  Blog-3-simple-steps.png
  Blog-Self-Employed.png
  Blog-Reverse-mortgage.png
  what-is-a-reverse-mortgage-how-does-it-work.png  (for "What Is a Reverse Mortgage and How Does It Really Work?" post)
  reverse-mortgage-myths-debunked.png  (for "7 Reverse Mortgage Myths..." post)
  Blog-DSCR.png
  Blog-California-1st-time-buyer.png

Names with spaces (use exactly as shown; URLs will be encoded):
  How much house can I afford_.png
  Rent or buy in SoCal_.png
  The Honest Truth About Down Payments for First-Time Buyers in Southern California.png
  What Credit Score Do You Actually Need to Buy a Home in Southern California.png
  CalHFA and Down Payment Assistance in Southern California A Complete Guide for First-Time Buyers.png

After adding the images, run: npm run build
